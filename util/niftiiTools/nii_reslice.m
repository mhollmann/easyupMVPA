function reslicedVolume = nii_reslice(spaceDefImage, imgToReslice, flags, resliceTransform)
...
% $Id: spm_reslice.m 1020 2007-12-06 20:20:31Z john $
  ...
  %use always b-spline interpolation
  flags.interp = 1;

  %get the voxel to world mapping of the file that should be resliced
  tResliceImg = spm_get_space(P{2});
  
  if iscell(P), P = strvcat(P{:}); end;
  if ischar(P), P = spm_vol(P); end;
  
  %now set the transformation to the image that should be resliced
  P(2).mat = resliceTransform*tResliceImg;
  reslicedVolume = reslice_images(P, flags);
 
  return;
%_______________________________________________________________________

%_______________________________________________________________________
function [resclicedVolume] = reslice_images(P,flags)
